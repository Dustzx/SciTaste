# Matched idea-to-paper package-preference prepilot governance v4
...
reviewer recruitment, or experiment cell. It supersedes v3 for new planning only;
v3 remains immutable historical evidence.
...
This v3 protocol governs the distinct **idea-to-paper research-package** lane.
...
DeepSeek's 2026-09-11 official catalog names the rolling callable identifier
`deepseek-flash` and served version `DeepSeek-V4.1-Flash`. It accepts the older
`deepseek-v4-flash` and `deepseek-v4-flash-vision-exp` identifiers only as
compatibility aliases routed to V4.1; they are not the pinned identity for this
proposal. DeepSeek V4.1 Flash and GLM-5.3-Flash require separate proposals,
hashes, dated pricing records, authenticated served-model observations, cells,
and estimates.

Qwen3-VL-2B-Instruct on eight RTX 3090 GPUs is a separate small-model or
experimental-workload robustness lane and cannot replace the frontier
matched-backbone lane.
...
## 11. Review-obligation mapping and ICLR evidence ladder

The internal DeepSeek V4.1 whole-paper review of draft v6 identified four
open concerns. `missing-effectiveness-evidence`, `missing-external-baseline`,
and `single-task-generalization` require new formal experiment evidence;
`overclaim-title-improvement` remains open until those results support the
fixed title or the final paper narrows its language. No prepilot report, model
review, implementation test count, or local small-model run closes them.

The evidence ladder is preregistered as follows:

1. one complete task/seed block across SciTaste Native, the direct-agent
   control, and at least two qualified accepted method comparators establishes
   only launch and adapter feasibility;
2. a clean multi-task prepilot estimates failure modes, effect variance,
   reviewer burden, and the formal sample size without establishing
   effectiveness;
3. a separately frozen formal study, powered over tasks and reviewed by at
   least two condition-blinded independent experts per artifact, may support
   the title-level improvement claim; and
4. the eight-RTX-3090 Qwen3-VL-2B lane is robustness and executor evidence only
   and is never pooled with the frontier API matched-backbone estimate.

Benchmarks and methods keep separate roles: MLR-Bench contributes starting
research briefs and review structure, while MLR-Agent, Agent Laboratory, and
any later admitted system are method comparators. Benchmark results are not
counted as an external-method comparison unless the real method implementation
runs under the frozen matched envelope. A completed formal result must then be
bound into a new paper revision and pass another internal response check plus
two independent human pre-submission reviews before the idea-to-paper loop is
reported complete.

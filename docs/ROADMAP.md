...
| Phase 9.23 — lifecycle Taste policy and self-revising Idea contract | in progress | the project verifies one current content-bound Idea revision and all of its inputs; external precedents, Tool Intelligence outcomes, and Generation as Content corrections converge on one quarantined, revision-bound episode schema; independent attribution review, source-group/split-aware admission and estimation, scientific-outcome filtering, causal controls, uncertainty abstention, and controller traces are implemented; next collect real longitudinal episodes and run held-out lifecycle evaluation before any old prepilot is treated as the title-level experiment |

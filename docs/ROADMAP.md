...
| Phase 9.31 — family-conditioned Scientific Taste | in progress; complete activation chain executable but unapproved | a seven-family scientific-judgment ontology, dual-AI assignment ledger, independently fitted uncertainty-aware heads, controller routing/trace, AI-reviewed grounded-corpus tier, and project-level refresh/readiness bundles are implemented. Policy v6 retains three scientific-value training episodes but its adaptive-allocation head has zero training episodes, zero source groups, and zero support; application therefore remains disabled. Activation v1 freezes nine unused NewtonBench modules, one outcome-independent post-feedback decision per task, cross-model review, threshold 3, and an exact E2-v5 target-domain probe. Exact approval leads to one-use trajectory permits in frozen order; only after all trajectories terminate do candidate-bound permits allow two attribution plus two outcome-blind family generations from revision-bound Qwen3-VL-4B/8B profiles. Dedicated zero-retry backends keep the maximum at exactly 36 attempts; rejection, disagreement, and runtime failure are retained without adjudication or replacement. The deterministic finalizer then seals eligible episodes, fits policy v7 at the unchanged threshold, and runs the five-state MLRC probe only when the adaptive head is support-ready. The 81-call/1,080,000-token API and eight-GPU-hour review ceilings remain unconsumed; neither refresh nor probe is an effectiveness result or E2 execution authority. |

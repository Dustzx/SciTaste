...
| Phase 9.23 — lifecycle Taste policy and self-revising Idea contract | in progress | the project verifies one current content-bound Idea revision and all of its inputs; external precedents, Tool Intelligence outcomes, and Generation as Content corrections converge on one quarantined, revision-bound episode schema; independent attribution review, source-group/split-aware estimation, scientific-outcome filtering, uncertainty abstention, and controller traces are implemented; a project-aware path now freezes prospective sampling, persists an exact natural state/decision/immediate-outcome foundation after source-run registration, reconstructs it without manufacturing a delayed scientific label, and deterministically joins later run-owned outcome evidence into a quarantined process episode; the production bridge prepares a shared evidence packet, records separate GLM/DeepSeek model-node calls, materializes all required attribution-review artifacts, admits a cross-model panel, and repeats an outcome-blind cross-model panel to assign the fixed decision family without a manual label, while retrospective self logs remain audit-only; next run the real cross-model review/admission path on natural candidates and then held-out lifecycle evaluation before any old prepilot is treated as title-level evidence |
...
| Phase 9.31 — family-conditioned Scientific Taste | in progress | a seven-family scientific-judgment ontology, dual-AI assignment ledger, independently fitted uncertainty-aware heads, explicit controller routing/trace, AI-reviewed grounded-corpus tier, CLI policy compiler, and H4 adaptive-allocation reproduction binding are implemented; next produce real admitted trajectory episodes and family assignments, then freeze the resulting project artifacts before the one final Python 3.12 integration/debug pass |

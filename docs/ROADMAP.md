...
| Phase 9.31 — family-conditioned Scientific Taste | in progress | a seven-family scientific-judgment ontology, dual-AI assignment ledger, independently fitted uncertainty-aware heads, controller routing/trace, AI-reviewed grounded-corpus tier, and project-level refresh/readiness bundles are implemented. Corpus v4 contains four independent source groups, including source-disjoint NewtonBench Gravity success and Fourier failure. Policy v6 uses signed pairwise credit: beneficial STOP contributes 0.5 win and harmful STOP contributes 0.5 loss, giving a neutral STOP posterior instead of falsely rewarding both outcomes. Three episodes train, but maximum feature support remains 1.0 below the frozen threshold 3, so application remains disabled. Next broaden source-disjoint support across both action types and decision families rather than lowering the gate. |
...
| Phase 9.36 — complete AutoResearch program and resource binding | implementation and one-shot B0 complete | one project-owned 17-state controller now covers acquisition, task admission, role-scoped model/system freeze, idea selection, experiment execution, hidden scoring, evidence admission, paper assembly, dual-AI review, adjudication, review-driven revision, and immutable package closure. The independent-judge successor B0 completed all 14 fixed task-excluded requests without selective reruns: Qwen3.8-Max is frozen for research/code, GLM-5.3-Flash for judge, MiniLM for retrieval, and Qwen3-VL-8B for the scoped local task role. The exact selection is bound into E2 v4; the incomplete predecessor campaign remains explicitly superseded. Qwen3-VL-2B is still a lower bound, inventory presence never selects a model, and B0 remains engineering-only rather than paper effectiveness evidence. |
| Phase 9.37 — capability-driven complete AutoResearch execution | in progress; v8 controller at development execution | v7 is retained as a zero-execution failed plan because its campaign and result-admission commands named different E2 identities. V8/E2-v4 is now the current self-project authority, binds one identity across execution and admission, and freezes an eight-call/193,536-token/USD-1/zero-retry API ceiling per block. SciTaste is one system; T0 training-free and T1 training-based name the research workloads it executes, while A0/A1 independently name its Taste adaptation mechanism. V8 replayed the existing acquisition, quarantine, split/firewall, model-role, prospective Idea, and experiment-plan artifacts without rerunning them and stops at `development-execution`. NewtonBench Gravity/Fourier remain development learning units, not Native/Base effects, and formal results remain zero. The next action is owner approval of the exact E2-v4 hash and two-arm MLRC development block, followed by its complete paper/review/revision trajectory; no further model-role B0 campaign is planned. |
...
declares `training-based-research`; its current E2 manifest passes all eleven
static scientific/execution boundaries, while model-role selection, the B0 GPU
pair, and execution authorization remain deliberately open. Neither track has a
formal Native/Base result yet.

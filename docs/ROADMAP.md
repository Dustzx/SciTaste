...
| Phase 9.29 — prospective atomic-calibration planning | in progress | v2 made exactly two real GLM-5.3-Flash calls and failed closed when 2/33 reported spans changed curly quotes to ASCII; no retry occurred, two independent AI audits accepted the failure handling, and the v2 sample is permanently consumed. Candidate v3 uses a new 24-item sample disjoint from all 36 prior items, a tracked consumed-sample registry, complete-decision rubrics, and only a precommitted equal-length quote/NBSP codepoint map with unique source re-anchoring; fuzzy, whitespace, hyphen, NFKC, insertion, and deletion repair remain forbidden. The runner now persists successful and failed per-call receipts plus failed sequence/request/response/status evidence. Next freeze v3 at a clean commit, compile its no-contact pack, bind a fresh authorization, and run once before independent AI integrity, authority, and semantic-completeness audits. |

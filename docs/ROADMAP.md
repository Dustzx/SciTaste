...
| Phase 9.23 — lifecycle Taste policy and self-revising Idea contract | in progress | the project verifies one current content-bound Idea revision and all of its inputs; external precedents, Tool Intelligence outcomes, and Generation as Content corrections converge on one quarantined, revision-bound episode schema; independent attribution review, source-group/split-aware estimation, scientific-outcome filtering, uncertainty abstention, and controller traces are implemented; a project-aware path freezes prospective sampling, persists exact natural state/decision/immediate-outcome foundations, reconstructs them without manufacturing delayed scientific labels, and deterministically joins later run-owned outcome evidence into quarantined process episodes; two real self-development decisions have completed cross-model attribution, outcome-blind family assignment, admission, explicit project-corpus sealing, and atomic policy refresh; the first was excluded as execution-only and the second remained below minimum independent support; next collect scientific-outcome episodes across the planned families and run held-out lifecycle evaluation before any prepilot is treated as title-level evidence |
...
| Phase 9.35 — prospective decision-episode cohort ingress | in progress | a fail-closed cohort layer now converts genuine prospectively frozen project decisions, delayed outcome proposals, uncertainty-preserving attribution, and completed cross-model AI review into source-group/split-disjoint Track-A inputs with a label-hidden target projection and scoring-only vault. Two real self-development episodes pass the development diagnostic lane, but remain excluded from external-validity evidence; formal external precedent and held-out target populations are still empty. H4 research-loop state/action/iteration/result locators have a pending adapter hook, and post-run scores cannot backfill predecision alternatives or outcomes. |

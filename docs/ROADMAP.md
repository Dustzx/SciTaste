...
| Phase 9.37 — capability-driven complete AutoResearch execution | in progress | v7 remains the self-project authority. SciTaste is one system; T0 training-free and T1 training-based name the research workloads it executes, while A0/A1 independently name its Taste adaptation mechanism. NewtonBench supplies a first-party T0 loop with hidden scorer, isolated code tool, condition-hidden interventions, immutable receipts, and separate prepare/run entrypoints. Gravity v7 succeeded and Fourier v8 was numerically accurate but failed exact symbolic scoring; the latter is retained as harmful STOP evidence rather than hidden. Identical-packet Qwen3-VL-4B/8B panels admitted and outcome-blind family-assigned both cases. Policy v6 now learns signed credit and correctly cancels the conflicting STOP evidence, while schema-aware local repair and diagnostic failure archives close two runtime gaps. These are development learning units, not Native/Base effects. Formal results remain zero; the next substantive step is a complete development-only Idea→experiment→paper→review→revision trajectory, followed by the smallest disclosed T0 and T1 Native/Base pairs. |

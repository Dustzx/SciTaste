...
| Phase 9.37 — capability-driven complete AutoResearch execution | in progress | v7 is the current self-project authority and chooses candidate model strata from the frozen Idea, recent neighboring AutoResearch work, and benchmark conventions before consulting available assets. Scientific agents, two independent reviewers plus adjudicator, embeddings, and benchmark task models are separate roles; Qwen3-VL-2B/4B are optional lower bounds only. The latest real B0 campaign passed research, code, retrieval, and local 8B task execution, while Qwen3.8-Max failed the judge reproducibility gate. Next expand and qualify the independent review panel, complete one development-only paper-review-return trajectory, then disclose and launch the exact Native/Base MLRC objective pair. |

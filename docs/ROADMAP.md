...
| Phase 9.29 — prospective atomic-calibration planning | in progress | a deterministic planner has frozen 24 previously unseen, source-balanced records; corrected protocol v2 and request pack v4 separate blind A/B segmentation from disputed-only anonymous C adjudication. The one-shot runner now uses a pre-call replay ledger, exact persisted request bytes, start/end identity sentinels, strict response/usage/span validation, zero retry, and a post-adjudication composite metric receipt. Official CNY rates remain separate from the owner's USD liability cap, and a successful run still cannot authorize population scale until two independent AI audits accept its integrity and authority boundaries. Next bind the committed runner and current official snapshots in one expiring authorization, execute only this calibration, then dispatch the two AI audits before any 273-source shard. |

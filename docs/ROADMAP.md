...
| Phase 9.39 — four-layer SciTasteBench evidence contract | design frozen; development cohort not yet materialized | the ICLR evaluation now separates source/abstraction (SA), held-out decisions (D), delayed-credit learning (L), and externally scored complete projects (P); replaces a flat accuracy headline with five causal endpoints, requires behavior-changing mediation for Taste attribution, distinguishes six lifecycle-context families from the existing seven-family Taste judgment ontology, moves Visual to product evidence, and freezes a 36-case natural development release before the 120-case formal floor. The historical 24-target pilot covers only four legacy context families and is retained as consumed development evidence. |
| Phase 9.40 — budgeted decision-regret execution | implementation complete; natural v4 cohort pending | SciTasteBench schema 4.0 can bind context and judgment as separate axes, record AI-panel rather than expert authority, bind hidden per-action utilities and abstentions, and report paired regret reduction. Legacy suite hashes remain stable; the next evidence action is to materialize the 36-case natural development cohort rather than run another engineering smoke campaign. |

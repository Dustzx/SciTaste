...
| Phase 9.31 — family-conditioned Scientific Taste | in progress; activation cohort executable but unapproved | a seven-family scientific-judgment ontology, dual-AI assignment ledger, independently fitted uncertainty-aware heads, controller routing/trace, AI-reviewed grounded-corpus tier, and project-level refresh/readiness bundles are implemented. Policy v6 retains three scientific-value training episodes but its adaptive-allocation head has zero training episodes, zero source groups, and zero support; application therefore remains disabled. Activation v1 freezes all nine previously unused NewtonBench modules in a balanced source-disjoint cohort, one outcome-independent post-feedback decision per task, cross-model AI review, threshold 3, and target-domain state probing. The typed no-run compiler verifies all task/source/model/checkpoint/resource identities; its successor plan also freezes the current Idea scientific contract. Protocol v1.1 enforces the earliest executed post-observation non-STOP sample and action-local projection rather than harvesting every turn. Exact owner approval leads to a no-run ready state, then one-use task permits and terminal accounting expose tasks strictly in order without selective reruns or effect-claim authority. Its 81-call/1,080,000-token API and eight-GPU-hour local-review ceilings are disclosed but not authorized or executed; the threshold cannot be lowered to force activation. |

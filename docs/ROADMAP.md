...
| Phase 9.40 — budgeted decision-regret execution | implementation complete; 36-case curation pending | SciTasteBench schema 4.0 can bind context and judgment as separate axes, record AI-panel rather than expert authority, bind hidden per-action utilities and abstentions, and report paired regret reduction. Legacy suite hashes remain stable. The causal interpretation is now frozen around five falsifiable signatures—structure, applicability, selective decision value, delayed credit, and progress efficiency—with explicit null/boundary conclusions. The next evidence action is model-assisted outcome-blind screening and source-alias resolution over the 141-item intake, followed by allocation and construction of exactly 36 development cases. |

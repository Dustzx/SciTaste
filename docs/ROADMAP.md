...
| Phase 9.37 — capability-driven complete AutoResearch execution | in progress | the v4 main authority supersedes the old checkpoint-bound prepilot for new runs, validates all research capabilities from goal/reference acquisition through hidden scoring, evidence-linked paper, independent review, revision, and final package freeze, and selects research/code/judge/retrieval/task models independently from task-excluded evidence. Qwen3-VL-2B is restricted to a low-cost lower bound; all qualified local/remote assets and task-fit downloads are eligible. Next finish B0 executor receipts and bind the first exact SciTaste Native/Base MLRC hidden-objective pair before real API/GPU execution. |

...
| Phase 9.35 — prospective decision-episode cohort ingress | in progress | a fail-closed cohort layer now requires a two-phase v2 chain: an outcome-free immutable predecision lock followed by a strictly later outcome attachment and completed projection. It converts uncertainty-preserving attribution and completed cross-model AI review into source-group/split-disjoint Track-A inputs with a label-hidden target projection and scoring-only vault. The two existing self-development captures remain useful historical dogfood but are pending as `legacy-temporal-unverified`; a genuinely new v2 self-development episode and formal external precedent/held-out populations are still needed. H4 research-loop state/action/iteration/result locators have a pending adapter hook, and post-run scores cannot backfill predecision alternatives or outcomes. |
...
| Phase 9.37 — capability-driven complete AutoResearch execution | in progress | v7 is the current self-project authority. The primary SciTaste lifecycle Taste policy is parameter-training-free, while a separate workload contract now requires both T0 training-free and T1 training-based complete research trajectories; SciTaste is not incorrectly split into two model-training methods. A typed candidate catalog starts from the two direct 2026 Scientific Taste neighbors, then uses AutoResearch methods and benchmark conventions to assign separate scientific-agent, Taste-baseline, coding, review, embedding, and benchmark-task roles. Sixteen candidates span fourteen families, including unavailable GPT/Gemini routes, both released 4B/30B Scientific Judge and Thinker checkpoints, and their exact architecture/scale Qwen bases; inventory is a feasibility/cost tiebreak only. The 4B comparisons are the minimum reproduction and the conditional 30B comparisons test scale-by-Taste-training robustness. Both 4B neighbor checkpoints are now project-bound and passed real task-excluded load/schema-generation B0 on separate RTX 3090s; their exact Qwen bases are being acquired before any native-task claim. A direct-neighbor extension separates native impact/ideation reproduction, lifecycle transfer, core mechanism contrasts, and downstream objective progress without aggregating them. The E2 handoff binds the catalog and accepted Idea, while Qwen3-VL-2B/4B remain optional lower bounds. Next complete the matched direct-neighbor base pair, select one objective T0 task, qualify the independent review panel, complete one development-only paper-review-return trajectory, then disclose and launch the exact Native/Base T0/T1 pairs. |

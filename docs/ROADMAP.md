...
| Phase 9.36 — complete AutoResearch program and resource binding | implementation and one-shot B0 complete | one project-owned 17-state controller now covers acquisition, task admission, role-scoped model/system freeze, idea selection, experiment execution, hidden scoring, evidence admission, paper assembly, dual-AI review, adjudication, review-driven revision, and immutable package closure. The independent-judge successor B0 completed all 14 fixed task-excluded requests without selective reruns: Qwen3.8-Max is frozen for research/code, GLM-5.3-Flash for judge, MiniLM for retrieval, and Qwen3-VL-8B for the scoped local task role. The exact selection is bound into E2 v3; the incomplete predecessor campaign remains explicitly superseded. Qwen3-VL-2B is still a lower bound, inventory presence never selects a model, and B0 remains engineering-only rather than paper effectiveness evidence. |
| Phase 9.37 — capability-driven complete AutoResearch execution | in progress; controller at development execution | v7 remains the self-project authority. SciTaste is one system; T0 training-free and T1 training-based name the research workloads it executes, while A0/A1 independently name its Taste adaptation mechanism. The controller has replayed existing acquisition, quarantine, split/firewall, and actual role-selection evidence, then prospectively locked the outcome-calibrated Idea from a three-candidate menu and frozen H0--H4 plus the E2 hidden-evaluation plan. It now stops at `development-execution`. NewtonBench Gravity/Fourier remain development learning units, not Native/Base effects, and formal results remain zero. The next action is the separately approved two-arm MLRC development block followed by its complete paper/review/revision trajectory; no further B0 campaign is planned. |

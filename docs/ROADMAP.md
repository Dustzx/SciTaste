...
| Phase 9.29 — prospective atomic-calibration planning | in progress | a deterministic planner has frozen 24 previously unseen, source-balanced records; a no-run GLM-5.3-Flash protocol records metrics, thresholds, identity-window, request/token/cost limits, and failure routing, and an external receipt binds it to clean commit `e843e3c` before provider contact; a typed inspector and eight-packet no-contact compiler expose only anonymous IDs, abstract, and review text. Independent AI review found that v1 conflated segmenter blinding with the adjudicator boundary and could authorize scale before adjudication. The new execution layer therefore remains no-live, requires a schema-1.1 disputed-only adjudication firewall, persists exact outbound bytes, and treats only the later post-adjudication composite gate as authoritative; next freeze v2, bind the committed runner and current price liability receipt, then execute only this calibration before any 273-source shard. |

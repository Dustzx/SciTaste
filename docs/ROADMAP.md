...
| Phase 8.16 — H1/H2 AI-only blind preference panel | done | the public blind package compiles and zero-retry executes two provider-distinct AI requests under exact token/cost limits, locks hash-bound raw HTTP/responses/receipts, routes only disputes to a third online or local identity, and opens the committed condition map only after final lock for an explicitly nonhuman paired endpoint |
| Phase 8.17 — AI-only grounded-Taste admission | done | accepted grounded-abstraction receipts now compile into two provider-distinct, generation-identity-blind review packs; exact response/receipt bytes lock before disputed-only third-model adjudication, and the final operational set explicitly forbids human or expert validity claims |
| Phase 8.18 — grounded-abstraction runtime/review bridge | done | a complete `BATCH.json` now admits only verified accepted live/local generations, binds ledger/result/recording/derived-receipt hashes, records rejected/failed/planned/missing exclusions and source coverage, and supports truthful API, local-model, or Codex-agent receipts without fabricating execution or human validity |
| Phase 9 — matched-budget study | in progress | AI-only operational review can unblock automation without a human-validity claim; executable matched-budget cells remain pending, while any external-human study is optional and separately labelled |
...
| Phase 9.2 — external-method archive qualification gate | done | the real 83,023,421-byte Agent Laboratory/DeepScientist transaction has a no-extraction 128,923,245-byte tree manifest; four exact dangling UI package-manager links are excluded, while unsafe/escaping/chained links remain blocked; extraction, installation, import, and execution remain closed |
...
| Phase 9.22 — first real native prepilot activation | in progress | the Perception nine-archive mapping passes license, receipt, ZIP-safety, materialization, and exact development/held-out isolation gates; its v2 adapter now adds a content-bound Python 3.12 import/NMS environment, label-free test-inference projection, exact-prediction scorer boundary, and fixed-upstream mAP parity; a five-arm matrix and multi-candidate claim contract encode Full-vs-Base, matched-Taste-vs-same-source-Raw-RAG, and matched-vs-mismatched-Taste without a policy confound; before activating even the complete five-arm Perception block, reproduce the actual GPU baseline/held-out candidate, curate formal contexts, bind v2 into a new campaign, disclose the full model/data/GPU budget, and request exact execution authority |
| Phase 9.23 — lifecycle Taste policy and self-revising Idea contract | in progress | the project verifies one current content-bound Idea revision and all of its inputs; external precedents, Tool Intelligence outcomes, and Generation as Content corrections converge on one quarantined, revision-bound episode schema; independent attribution review, source-group/split-aware estimation, scientific-outcome filtering, uncertainty abstention, and controller traces are implemented; a project-aware path freezes prospective sampling, locks exact outcome-free natural state/decision foundations, and joins strictly later run-owned outcome evidence without rewriting the predecision; two earlier one-phase self-development decisions completed cross-model attribution, outcome-blind family assignment, admission, corpus sealing, and policy refresh, but now remain temporal-unverified historical dogfood rather than prospective training evidence; next capture new v2 scientific-outcome episodes across the planned families and run held-out lifecycle evaluation before any prepilot is treated as title-level evidence |
...
| Phase 9.29 — prospective atomic-calibration planning | in progress | v2 made two real GLM-5.3-Flash calls and failed on 2/33 quote-normalized spans; frozen v3 then used a disjoint 24-item sample and made five real calls, but failed when B2 removed source tokenization spaces and Markdown emphasis in 3/31 locators. Both runs stopped without retry and both samples are immutably consumed. A two-agent AI audit additionally disclosed that v3's embedded YAML times postdate provider contact; immutable Git history proves the actual pre-contact byte order, but those timestamps cannot support a preregistration claim. The repeated locator failure rejects the premise that a generative model should reproduce source bytes: next freeze a deterministic high-resolution evidence-unit contract and run offline adversarial tests before selecting another sample. |
| Phase 9.30 — native lifecycle-policy H4 execution | in progress | the native runner now consumes a protocol profile plus a replayed five-state manipulation report, creates campaign-bound arm requests before baseline/model/mutation, selects one high-level action through deterministic Taste on every round, retains typed terminal evidence and persistent task-seed-repetition pairs, closes public-analysis legacy/pair/evidence-chain bypasses and selective primary reruns, hides operational IDs from the model, reserves full-campaign budget, and supports task-specific guidance. E2 schema 1.1 now refuses execution unless the exact adaptive-allocation head is support-ready, source-disjoint, and proven active plus feedback-sensitive by an outcome-blind target-domain state probe. Formal launch still requires the real policy, accepted Idea, frozen artifacts, disclosed resources, and owner-authorized cells. |
| Phase 9.31 — family-conditioned Scientific Taste | in progress; approval-to-E2 chain executable but unapproved | a seven-family scientific-judgment ontology, dual-AI assignment ledger, independently fitted uncertainty-aware heads, controller routing/trace, AI-reviewed grounded-corpus tier, and project-level refresh/readiness bundles are implemented. Policy v6 retains three scientific-value training episodes but its adaptive-allocation head has zero training episodes, zero source groups, and zero support; application therefore remains disabled. Activation v1 freezes nine unused NewtonBench modules, one outcome-independent post-feedback decision per task, cross-model review, threshold 3, and an exact E2-v5 target-domain probe. Exact approval now enters one append-only command that resumes sealed state and existing terminal task evidence, issues one-use permits in frozen order, never retries an interrupted generation, and closes an all-zero-candidate cohort without invented reviews. Candidate-bound permits allow two attribution plus two outcome-blind family generations from revision-bound Qwen3-VL-4B/8B profiles only after every trajectory terminates. Dedicated zero-retry backends keep the maximum at exactly 36 attempts; rejection, disagreement, and runtime failure are retained without adjudication or replacement. The deterministic finalizer seals eligible episodes, fits policy v7 at the unchanged threshold, and runs the five-state MLRC probe only when the adaptive head is support-ready. A passing finalization automatically compiles a new E2 successor with policy/readiness/probe bytes and E2-v5 as its acyclic activation basis; benchmark launch still needs a separate exact owner approval. The 81-call/1,080,000-token API and eight-GPU-hour review ceilings remain unconsumed; neither refresh nor probe is an effectiveness result. |
...
| Phase 9.33 — independent SciTasteBench validation reserve | paused after consumed v7 diagnostic | two exact official-source acquisitions provide 40 F1000 and 20 ARIES dev source groups disjoint from earlier calibration sources; v6 and v7 each exposed a runner-contract strictness issue rather than model or Taste quality, and both samples are permanently consumed; retain the remaining 10 ARIES plus 30 F1000 groups, stop calibration churn, and prioritize the deadline-critical Track-A H1/H2 pilot |
| Phase 9.34 — bounded Track-A decision execution | implementation complete; real execution not started | a strict no-call compiler turns the coverage-aware three-arm suite into exact condition-hidden model requests only after all eligible inputs have tokenizer traces and hard call/token/cost/byte/latency budgets; live execution is double-opt-in, zero-retry, terminal on any failure, and never replaces missing cases. A complete eligible-subset run can feed the existing two-primary plus disputed-only-adjudicator AI panel, with planned 24/72, eligible, and executed counts kept distinct and no human-validity claim. |
| Phase 9.35 — prospective decision-episode cohort ingress | in progress | a fail-closed cohort layer requires an outcome-free immutable predecision lock followed by a strictly later outcome attachment and completed projection. NewtonBench Gravity v7 and source-disjoint Fourier v8 now supply one beneficial and one harmful external-task STOP episode, each admitted by an identical-packet cross-model panel and outcome-blind family assignment. Long trajectories use a compact, hash-bound outcome projection while retaining the full immutable receipt as source authority. The two earlier self-development captures remain historical dogfood; formal precedent/held-out populations and broader decision-family coverage remain open. |
| Phase 9.36 — complete AutoResearch program and resource binding | implementation and one-shot B0 complete | one project-owned 17-state controller now covers acquisition, task admission, role-scoped model/system freeze, idea selection, experiment execution, hidden scoring, evidence admission, paper assembly, dual-AI review, adjudication, review-driven revision, and immutable package closure. The independent-judge successor B0 completed all 14 fixed task-excluded requests without selective reruns: Qwen3.8-Max is frozen for research/code, GLM-5.3-Flash for judge, MiniLM for retrieval, and Qwen3-VL-8B for the scoped local task role. The exact selection is retained in E2 v5; the incomplete predecessor campaign remains explicitly superseded. Qwen3-VL-2B is still a lower bound, inventory presence never selects a model, and B0 remains engineering-only rather than paper effectiveness evidence. |
| Phase 9.37 — capability-driven complete AutoResearch execution | in progress; v9 controller at development execution | v7 is retained as a zero-execution failed plan because its campaign and result-admission commands named different E2 identities; v8 is retained as a zero-execution failed plan because it did not prove a behaviorally active Taste treatment. V9/E2-v5 is the current self-project authority: execution/admission identity and Qwen3.8-Max budgets remain closed, but an ON arm cannot launch until learned Taste is support-ready, state-sensitive, and measurably behavior-changing. SciTaste is one system; T0 training-free and T1 training-based name its workloads, while A0/A1 name adaptation. Formal results remain zero. The next external action is the separately disclosed adaptive-allocation activation cohort; after its target-domain state probe passes, run one two-arm MLRC development pair and then the formal pair. No further model-role B0 campaign or repeated smoke layer is planned. |
| Phase 9.38 — resumable Track-A abstraction campaign | implementation complete; next source-disjoint batch unapproved | one batch-level command now discovers immutable model-node ledger entries, executes only missing frozen abstractions behind explicit live authority, resumes interrupted evidence without retrying any terminal invocation, retains accepted/rejected/failed/not-applicable/planned outcomes, and automatically compiles the independent AI-review pack once at least one result is admissible. Read-only inspection of the prior quality-qualified batch reports 3/3 consumed, 2 runtime-accepted, 1 rejected, 13,622 tokens, USD 0.001538, and zero new calls; its later AI review accepted 0/2, so no H1/H2 effect follows and a new preregistered source-disjoint batch is still required. |
| Phase 9.39 — four-layer SciTasteBench evidence contract | design frozen; natural intake materialized | the ICLR evaluation now separates source/abstraction (SA), held-out decisions (D), delayed-credit learning (L), and externally scored complete projects (P); replaces a flat accuracy headline with five causal endpoints, requires behavior-changing mediation for Taste attribution, distinguishes six lifecycle-context families from the existing seven-family Taste judgment ontology, moves Visual to product evidence, and freezes a 36-case natural development release before the 120-case formal floor. A no-call intake verified 370 natural candidates, projected one outcome-hidden representative from each of 141 source groups, retained later outcomes in a scoring-only vault, and found 101 groups with no exact prior Track-A role across three domain strata. The historical 24-target pilot remains consumed development evidence and no case has yet been promoted. |
| Phase 9.40 — budgeted decision-regret execution | implementation complete; dual screening complete; third-panel plan frozen | SciTasteBench schema 4.0 can bind context and judgment as separate axes, record AI-panel rather than expert authority, bind hidden per-action utilities and abstentions, and report paired regret reduction. Two independent, outcome-blind, zero-retry screens covered all 101 unused source groups: Qwen3.8-Max proposed 84 eligible cases for 132,812 tokens and USD 2.096904, while GLM-5.3-Flash proposed 77 for 135,621 tokens and a conservative USD 1.648568 ceiling. Deterministic all-batch normalization fixed only model-copied envelope identities and retained original rejected receipts. Of 71 jointly eligible cases, context labels agree on 40; resource-allocation has zero agreement and only a five-case union, so no quota has been fabricated. The third-model operator now bypasses 21 exact agreements and 11 joint exclusions and freezes 69 disputes as nine ordinal-position DeepSeek-V4.1-Flash batches. Its first local launch stopped before transport at zero tokens/cost because no `DEEPSEEK_API_KEY` is stored; retain that receipt and create a new version after credential binding rather than retrying it. Then allocate and construct exactly 36 cases. |

The first real NewtonBench T0 development trajectory completes experiment
...
`symbolic_accuracy=1.0`. Its STOP-credit candidate is now admitted as
`newton-gravity-v7-stop-scientific-credit-v3` (admission SHA-256
`064c4dcb74bb2b9c93adacdbf444d656e34e5ed330286e858f4db0b16a78d602`).
Qwen3-VL-8B and Qwen3-VL-4B independently received the same frozen packet and
agreed on STOP plus `interactive-local-credit-002`; confidence is conservatively
0.5. A source-disjoint Fourier v8 trajectory then used 21 experiments and
terminated after five turns. Its numerical fit was close, but the frozen hidden
scorer returned `symbolic_accuracy=0.0`, so the STOP decision was assigned
harmful rather than beneficial credit. A compact outcome projection bound to the
full receipt kept both local reviewers inside context; both accepted the same
packet and outcome-blind assigned `scientific-value`. Corpus v4 therefore holds
four source groups and policy v6 trains on three eligible episodes. Its signed
estimator records STOP as 0.5 win and 0.5 loss, posterior mean 0.5. Maximum
feature support is only 1.0 versus the frozen threshold 3, so the controller
continues to abstain. Both Newton trajectories are development learning units,
not formal Native/Base effects.
...
locked Taste action terminates before tool or scorer execution. The retained
gravity bootstrap failure was an agent inference/compliance failure, not a
changed hidden law, and is forbidden from policy fitting. T1 MLRC Perception now
declares `training-based-research`; E2 v5 closes the exact treatment identity
but reports the adaptive-allocation head as behaviorally inactive. The fixed
activation cohort, one MLRC development pair, and later formal pair remain
deliberately unexecuted. Neither track has a formal Native/Base result yet.
...
the required per-image license records. No model call, GPU job, or experiment
has occurred. A bounded 69-MiB Python 3.12 overlay now imports the real task and
executes compiled `nms_1d_cpu` on CPU. The v2 held-out projection removes all
labels from the candidate process, while an independent scorer requires exact
prediction and label hashes. Its 63-label, 126-prediction parity fixture matches
fixed-upstream mAP per threshold within `1e-15`; an empty-prediction real-label
negative control scores zero. This marks the adapter and scorer ready, not the
upstream Python 3.11 environment or GPU baseline reproduced. Those numerical
execution checks remain prelaunch gates.
...
retrieval. The executable schema-1.1 matrix uses five arms and excludes
Knowledge-only/Critics-only repetitions from the required allocation. This
reduces compute while making H1/H2 identifiable; optional component diagnostics
can be added only after the confirmatory cells are viable.
...
The provider changed the route again with the 2026-09-10 V4.1 release.
Resource catalog v13 therefore makes canonical `deepseek-flash` /
`DeepSeek-V4.1-Flash` current and demotes the retired `deepseek-v4-flash` V4
record to historical status. The legacy name is only a temporary compatibility
route to V4.1, so it cannot define a separate experimental stratum. Existing V4
proposals stay immutable and non-launchable; new work binds the V4.1 route and
uses peak pricing as its admission ceiling.
...
- A deterministic static rejection can now trigger exactly one separately
  ...
  A separately configured runtime-repair mode now consumes only a bound failed
  isolation record, stderr digest/excerpt, failure class, and original source
  identity. It makes one source-only proposal, preserves the first attempt,
  repeats the same admission/isolation contract, and resumes the remaining
...
- Done: selected fixed project labels can be proactively generated by the live
  model into a project-owned cache, then promoted as the exact immutable first
  turn of an editable project conversation without a second provider call.
  Follow-up feedback resumes from that cited model brief. Provider/model,
  intent, snapshot, TTL, and cumulative call/token/cost ceilings are explicit;
  failed calls debit their declared envelope. Tool Intelligence retains the
  owner boundary only for the paid secret-bearing warm pass, while reads and
  misses remain direct/on-demand.
- Done: the default project surface begins with a two-plane operating map rather
  than another long text synopsis. It shows model synthesis/edit/publication on
  one side and the current SciTaste evidence gate, Tool Intelligence route,
  next action, and project resources on the other. Plan and resource controls
  feed the typed model-revision path; detailed ledgers are retained in the
  collapsed evidence vault.
- Done: optional verification now chooses the smallest sufficient check. A full

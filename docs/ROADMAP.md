...
| Phase 9.37 — capability-driven complete AutoResearch execution | in progress | v7 is the current self-project authority. A typed candidate catalog now starts from the two direct 2026 Scientific Taste neighbors, then uses AutoResearch methods and benchmark conventions to assign separate scientific-agent, Taste-baseline, coding, review, embedding, and benchmark-task roles. Ten candidates span eight families, including unavailable GPT/Gemini routes; inventory is a feasibility/cost tiebreak only. The E2 handoff binds this catalog and the accepted Idea, while Qwen3-VL-2B/4B remain optional lower bounds. The latest real B0 campaign passed research, code, retrieval, and local 8B task execution, while Qwen3.8-Max failed the judge gate. Next qualify the independent review panel and direct-neighbor Taste baselines, complete one development-only paper-review-return trajectory, then disclose and launch the exact Native/Base MLRC objective pair. |

...
The current multidomain self-use pass closes an important flexibility detail.
One fixed Taste-curation label can pre-cache model-authored content across the
ARIES and F1000 project evidence, while an unforeseen question still receives a
fresh generated brief/canvas and feedback edits the exact prior artifact. The
receiver tolerates only truthful redundant display echoes and narrows visual
focus to cited candidate evidence; it does not relax claim citations or accept
...
deterministic policy's gray zone. The same Generation as Content call may return
that advice over the exact server-issued verification fingerprint, so no second
model review is added; the controller recomputes policy and rejects any attempt
to weaken a hard boundary. Schema-rejected provider responses retain known usage
and cost and are negative-cached for the exact request, while no-response
transport failures remain retryable. The first policy uses disclosed priors and
...
The same rule now covers natural-reference acquisition, independent-review
return collection, and grounded-input compilation. Exact
...
The rule also governs asynchronous human evidence. Importing one reviewer return
does not receive a ceremonial preview: it follows `direct_path`, validates the
exact authorized session and complete response set inline, and updates the
project immediately. The third return triggers the deterministic locked-result
compiler. Recruitment stays behind `owner_approval`; abstraction, benchmark
admission, and experiments remain later decisions. This extends Tool
Intelligence from experiment gates into a real human-in-the-loop block without
turning every state change into a preflight.

The F1000 multidomain compiler exercised this policy on 77 natural episodes: its
reversible local transformation took `direct_path`, with XML bounds, privacy
transformation, hashes, and atomic writes performed inline. In parallel, model
judgment handled flexible project synthesis and feedback editing. This is the
intended division of execution intelligence—spend semantic capacity on ambiguous
decisions, not on a ceremonial preview of cheap deterministic work—while paid
API calls and GPU/formal experiments keep their bounded or owner-gated controls.

...
|---|---|---|---:|---|
| `native-taste-causal-prepilot-v10` | within-SciTaste causal feasibility | one content-bound Qwen3-VL-2B tree (`47f9c0e0...`) and one remote 8×RTX 3090 lane shared by all six conditions | 6 native conditions × 2 MLRC task candidates × 1 seed = 12 trajectories | current schema-v1.4 no-run proposal; six structural implementations bind commit `5bbf716...`, while v2 preflight qualifies bounded candidate generation plus fixed-candidate selection; Full--Base and Full--mismatched are the two confirmation obligations, while three component-only analyses are mandatory diagnostics; formal paired corpora, assets, remote checkpoint, reviewers, and approval remain blocked |
...
.venv/bin/scitaste evaluation prelaunch \
  --manifest configs/evaluation/prelaunch/qwen3vl2b_native_taste_causal_prepilot_v10.yaml \
...
provider call, GPU work, or task download occurred. The current native v10 and
...
.venv/bin/scitaste evaluation native-condition-preflight \
  --manifest configs/evaluation/preflight/qwen3vl2b_native_condition_path_v2.yaml \
  --source-root .
```
...
2. The dual-estimand architecture is materialized as native v10 and external v7

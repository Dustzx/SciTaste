...
- The local Generation-as-Content workspace provides project homes, conversations,
  immutable turns, generated surfaces, feedback-edited plans, and configuration controls.
- Every run, paper, review, evaluation, and interface surface belongs beneath one
  `outputs/projects/<project-id>/` tree.
- Shared resources remain in a secret-free registry; each project receives a versioned
  binding and can apply priority plans without exposing credentials or launching work.

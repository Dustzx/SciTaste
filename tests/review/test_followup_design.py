...
_ACTIVATION_V5 = _ROOT / "configs/evaluation/activation/iclr2027_review_followup_v5.yaml"
_SHA = "a" * 64
_COMMIT = "b" * 40
...
def test_latest_review_activation_binds_current_deepseek_v4_without_authority() -> None:
    design = compile_review_followup_design(
        project_id=_PROJECT,
        run_id="followup-design-v1",
        source_commit=_COMMIT,
    ...
    activation = compile_review_followup_activation(
        design=design,
        manifest_inspection=load_review_followup_activation_manifest(_ACTIVATION_V5),
        workspace_root=_ROOT,
        run_id="followup-activation-v5",
        source_commit=_COMMIT,
    )

    candidates = {item.resource_id: item for item in activation.primary_model_candidates}
    assert activation.model_identity_protocol_id == "iclr2027-api-identity-v3"
    assert set(candidates) == {"deepseek-v4-flash", "zhipu-glm53-flash"}
    assert candidates["deepseek-v4-flash"].model_id == "deepseek-v4-flash"
    assert candidates["deepseek-v4-flash"].declared_revision == "DeepSeek-V4-Flash"
    assert candidates["deepseek-v4-flash"].pilot_proposal_ready is True
    assert activation.primary_model_id is None
    assert activation.authorizes_api_calls is False
    assert activation.authorizes_execution is False
